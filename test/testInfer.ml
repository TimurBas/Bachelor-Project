...
let%expect_test "Non_polymorphic_id_example" =
  print_tau_node (infer_type non_polymorphic_id_example);
  TypeEnv.reset ();
  [%expect {| 1 -> 2 -> 3 -> 3|}]

let%expect_test "Polymorphic_id_example" =
  print_tau_node (infer_type polymorphic_id_example);
  TypeEnv.reset ();
  [%expect {| 2 -> 2 |}]

let%expect_test "Nested_let_example" =
  print_tau_node (infer_type nested_let_example);
  TypeEnv.reset ();
  [%expect {| (9 -> 10 -> 11 -> 12 -> 12) x (13 -> 14 -> 15 -> 16 -> 16) |}]

let%expect_test "Fun_application_example" =
  print_tau_node (infer_type fun_application_example);
  TypeEnv.reset ();
  [%expect {| (2 -> 3) -> 2 -> 3 |}]

let%expect_test "Fun_application_three_example" =
  print_tau_node (infer_type fun_application_three_example);
  TypeEnv.reset ();
  [%expect {| (2 -> 4) -> 2 -> (4 -> 5) -> 5 |}]

  let%expect_test "Tuple_fun_application_example" =
  print_tau_node (infer_type tuple_fun_application_example);
  TypeEnv.reset ();
  [%expect {| (2 -> 4) -> 2 -> 4 x 4 |}]

let%expect_test "Lambda_outside_let_example" =
  print_tau_node (infer_type lambda_outside_let_example);
  TypeEnv.reset ();
  [%expect {| 1 -> 2 -> (1 -> 6) -> 6 x 6 |}]

let%expect_test "Fst_lambda_example" =
  print_tau_node (infer_type fst_lambda_example);
  TypeEnv.reset ();
  [%expect {| 1 -> 1 |}]

let%expect_test "Fst_let_example " =
  print_tau_node (infer_type fst_let_example);
  TypeEnv.reset ();
  [%expect {| 1 -> 2 -> 2 |}]

let%expect_test "Everything_example" =
  print_tau_node (infer_type everything_example);
  TypeEnv.reset ();
  [%expect {| (5 -> 7) -> 5 -> 5 -> 7 x 7 |}]

let%expect_test "Everything_example" =
  print_tau_node (infer_type everything_example2);
  TypeEnv.reset ();
  [%expect {| 1 -> (1 -> 7) -> (1 -> 9) -> 7 x 9 |}]

let%expect_test "Polymorphic_id_with_int_and_bool" =
  print_tau_node (infer_type polymorphic_id_with_int_and_bool);
  TypeEnv.reset ();
  [%expect {| int x bool |}]

let%expect_test "Many_nested_let" =
  print_tau_node (infer_type many_nested_lets);
  TypeEnv.reset ();
  [%expect {| ((((((52 -> 52) x (53 -> 53)) x (54 -> 54)) x (55 -> 55)) x (56 -> 56)) x ((57 -> 58 -> 59) -> 57 -> 58 -> 59)) x (60 -> 60) |}]

let%expect_test "Many_lambdas" =
  print_tau_node (infer_type many_lambdas);
  TypeEnv.reset ();
